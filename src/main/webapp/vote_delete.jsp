...
<main class="page">
    ...
    <section class="panel">
        <% if (request.getAttribute("error") != null) { %>
            ...
            <a class="button secondary" href="<%= request.getContextPath() %>/vote/list">返回列表</a>
        ...
        <% } else { %>
            ...
            <form method="post" action="<%= request.getContextPath() %>/vote/delete">
                <input type="hidden" name="questionId" value="<%= question.getId() %>">
                <button class="danger" type="submit">确认删除</button>
                <a class="button secondary" href="<%= request.getContextPath() %>/vote/list">取消</a>
            </form>
        <% } %>
    </section>
</main>
